...
export const StudentDetailClient: React.FC<Props> = ({ studentId, role, salesReps }) => {
  ...
  const assignStudent = async (salesId: string | null) => {
    setAssigning(true);
    try {
      const res = await fetch(`/api/admin/students/${studentId}`, {
        method: 'PATCH',
      ...
      if (!res.ok) throw new Error(responseData.error || 'Failed to reassign');
      toast.success(salesId ? 'Sales rep assigned!' : 'Assignment removed');
      fetchDetail();
    } catch (err: any) {
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm text-slate-600 font-medium mb-6">
              <div className="flex items-center gap-1.5">
                <Mail className="w-4 h-4 text-slate-400" /> {student.email || 'No email'}
              </div>
              <div className="flex items-center gap-1.5">
                <Phone className="w-4 h-4 text-slate-400" /> {student.phone || 'No phone'}
              </div>
              <div className="flex items-center gap-1.5">
              ...
              <div className="mt-6 pt-6 border-t border-[#f0ebe2]">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      disabled={assigning}
                      value={student.assigned_to || ''}
                      onChange={(e) => assignStudent(e.target.value || null)}
                      className="px-3 py-2 rounded-lg border border-indigo-200 text-sm font-bold bg-indigo-50 outline-none cursor-pointer focus:border-indigo-500 disabled:opacity-50"
                    >
                ...
                {student.assigned_sales && (
                  <p className="text-[11px] text-slate-500 font-medium mt-2">
                    ...
                    <span className="font-bold text-indigo-700">
                      {student.assigned_sales.full_name}
                    </span>
                  </p>
                )}
                {!student.assigned_to && (
                  <p className="text-[11px] text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-2 py-1 mt-2 inline-block font-bold">
                    ⚠️ Not assigned to any sales rep
                  </p>
                )}
              </div>
            )}
            ...
            {courses.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100">
                {courses.map((c: any) => (
                  <div
                    key={c.course_id}
                    className={`p-5 ${!c.enrollment_active ? 'opacity-50 grayscale' : ''}`}
                  >
                    <div className="flex items-start gap-4">
                      <CourseThumbnail
                        thumbnailKey={c.course_thumbnail}
                        alt={c.course_title}
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          ...
                          {canManage && c.enrollment_active && (
                            <button
                              onClick={() =>
                                revokeAccess('course', c.course_id, c.course_title)
                              }
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span
                        className={`text-sm font-black ${
                          q.passed ? 'text-emerald-700' : 'text-slate-700'
                        }`}
                      >
                        {q.score}%
                      </span>
