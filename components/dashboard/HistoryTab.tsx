...
export const HistoryTab: React.FC<Props> = ({ payments }) => {
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 sm:p-12 text-center animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row gap-2 justify-center">
          <Link
            href="/courses"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#07CCFD] text-[#0F172A] text-sm font-bold cursor-pointer transition-all"
          >
  ...
  return (
    <div className="animate-fadeIn space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto hide-scrollbar bg-white border border-slate-200 rounded-xl p-1.5">
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap cursor-pointer transition-all ${
        active ? activeColors[color] : 'text-slate-600 hover:bg-slate-50'
      }`}
    >
      <span>{label}</span>
      {count > 0 && (
      ...
      <div className="flex items-start justify-between gap-4 mb-2 flex-wrap">
        ...
        {payment.status === 'approved' && payment.item_type === 'course' && (
          <Link
            ...
            className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#07CCFD] hover:bg-[#06B8E4] text-[#0F172A] text-xs font-bold cursor-pointer transition-all"
          >
            <span>Open</span>
